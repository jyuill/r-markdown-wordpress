---
title: "Using R for Google Trends Reporting and Analysis"
...
## 2. preferred/recommended: in code 
knitr::include_graphics('images/google-trends-interface-page-crypto.png')
```
...
include_graphics('images/google-trends-search-type-example-full.png')
```
...
```{r, echo=FALSE, fig.width=400, caption='google trends search types'}
include_graphics('images/google-trends-topic-crypto-bitcoin.png')
```
...
```{r google-trends-topics, fig.height=3, fig.width=8}
## create chart based on search interest over time
gt_interest %>% ggplot(aes(x=date, y=hits, color=keyword))+geom_line(size=2)+
  scale_y_continuous(expand=expansion(add=c(0,0)))
```
